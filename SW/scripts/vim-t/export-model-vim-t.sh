...
LOG_FILE="${SCRIPT_DIR}/export-model-vim-t.txt"
...
DATA_PATH="$DATA_PATH"
PTH_PATH="$VIM_T_PTH"
ACT_SCALE_PATH="$ACT_SCALES_BASE"
NUM_WORKERS="$NUM_WORKERS"
CUDA_DEVICES="$CUDA_DEVICES"

CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" log_output python e_export_model.py --eval \
    --model $MODEL \
    --batch-size 1 \
    --num_workers $NUM_WORKERS \
    --data-path "$DATA_PATH" \
