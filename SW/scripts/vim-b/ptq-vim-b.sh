...
LOG_FILE="${SCRIPT_DIR}/ptq-vim-b.txt"
...
CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" log_output python a_generate_act_scale.py \
  --resume "$PTH_PATH" \
  --model "$MODEL" \
  --data-path "$DATA_PATH" \
...
CUDA_VISIBLE_DEVICES="1,2" log_output python -m torch.distributed.launch --nproc_per_node=2 --use_env main.py --eval \
  --model "$MODEL" \
